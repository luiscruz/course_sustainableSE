...
Modern ad blockers operate as browser extensions that intercept and filter web content before it is rendered. Most ad blockers rely on rule-based filter lists that match outgoing network requests against known advertisement and tracking domains. When a request matches a rule, the browser prevents the associated resource from being downloaded or executed. In Chromium-based browsers, this functionality is typically implemented through extension APIs that allow blocking or modifying HTTP requests at the network level. Because online advertisements frequently include JavaScript code that dynamically loads additional scripts, performs tracking operations, and updates the page structure, preventing these requests can reduce script execution, DOM manipulation, and layout recalculations. At the same time, the filtering process itself introduces a small computational overhead due to rule matching and request inspection. The overall energy impact therefore depends on the balance between this filtering overhead and the reduction of advertisement-related processing.

On the one hand, an ad blocker introduces additional processing inside the browser, which could increase computational overhead. On the other hand, blocking advertisements and third-party scripts may reduce the amount of content loaded and executed, potentially decreasing CPU workload and energy usage. Given these opposing effects, the net impact on energy consumption is not immediately obvious. This raises an interesting question: does enabling an ad blocker increase or decrease energy consumption during web browsing?
