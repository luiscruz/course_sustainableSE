...
Several tools aim to improve sustainability in software systems. Electricity Maps provides carbon intensity data and APIs for integrating emissions awareness into applications. Cloud providers such as AWS and Azure expose region-level sustainability metrics, enabling coarse-grained workload placement decisions.

### Eco CI

A complementary approach to temporal scheduling is measuring the energy footprint of individual CI jobs. The Eco CI energy estimation tool [Eco CI 2024] addresses this by integrating energy measurement into GitHub Actions workflows. Eco CI tracks CPU utilization across job execution using pre-calculated power consumption curves based on hardware specifications, enabling developers to quantify the energy and CO₂ emissions of specific workflow steps. By instrumenting workflows with Eco CI's measurement actions (`start-measurement`, `get-measurement`, `display-results`), teams can identify the most energy-intensive jobs (such as long-running test suites or compilation steps) and prioritize those for deferral via Flow's scheduler. The combination of per-job measurement (Eco CI) and temporal load shifting (Flow) provides complementary insights: identifying *what* to optimize and *when* to optimize it respectively.
...
Given these parameters, our scheduler adopts the following strategy. When an idle repository receives a task, the system identifies a target activation time $t_{opt}$ corresponding to the predicted global minimum of carbon intensity within the interval $[t_{arrival}, t_{arrival} + 24H]$. To maintain accuracy against fluctuating forecasts, the scheduler re-evaluates the carbon forecast at periodic intervals, dynamically adjusting $t_{opt}$ if a more efficient execution window emerges.
