...
### Future work
Since we solely compared the power output of 3 models there is a large variety of options that could be available for future work. The first is the most straightforward, namely, comparing a larger variety of models, with more varied parameters as well. The models we looked at gave a good idea of their power usage, however, 7 billion parameter models are used in much smaller quantities, as their performance typically does not measure up to their larger counterparts in exchange for their efficiency. Additionally, there are models such as ChatGPT and Gemini which may be more commonly used, compared to the open-source alternatives we investigated [(First Page Sage. “Top Generative AI Chatbots.” First Page Sage, February 21, 2024.)](https://firstpagesage.com/reports/top-generative-ai-chatbots/). 

Furthermore, the back end that is used to make the API calls to the models is another aspect that, although smaller in impact, could benefit from further research. It has become increasingly common to utilize models locally, and as such open-source back-ends will see increased use as well. Different back-ends have minimal to no impact on the model output itself, however, many optimizations can be done to improve the energy consumption, such as through concurrent batch processing. Furthermore, as models become more commonly accessed through mobile devices such as mobile phones, there is considerable merit in optimizing the energy consumption from battery preservation on these devices, as they typically have greatly reduced battery capacity compared to laptops. The more energy-efficient ARM architectures, as opposed to the x86-based architectures that are frequently used by laptops, may also impact how back-ends affect energy consumption.
